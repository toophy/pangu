...
func (a *Actor) Base_moveTo(s *help.Vec3, d *help.Vec3, speed float32) bool {
	if a.MoveNode.IsEmpty() {
		l := float32(a.SrcPos.Dist(&a.DstPos))
		if l > 0.01 {
			// 移动新增加
			a.MoveLen = l
...
func (a *Actor) Base_onMove() bool {
	// 本线段上总共移动的距离
	s := float32(float32(a.CurScreen.Get_thread().GetCurrTime()-a.LastMoveTime) * a.MoveSpeed)
	...
	// 移动继续
	var n help.Vec3
	n.X = a.SrcPos.X + s*a.MoveRate.X
	n.Y = a.SrcPos.Y + s*a.MoveRate.Y
	n.Z = a.SrcPos.Z + s*a.MoveRate.Z
